"""Compare SGD and Gaussian S-SAM on an affine linear-advection PINN."""
...
CONFIG = {
    ...
    "pinn": {
        ...
        "output_dir": "outputs/advection_pinn",
        "evaluation": {
...
def main() -> None:
    ...
    points = build_advection_points(reference_config)
    ...
    for algorithm, config in configs.items():
        ...
        results[algorithm] = train_advection_pinn(model, points, config)
        metrics[algorithm] = evaluate_advection_pinn(
            model,
            points,
